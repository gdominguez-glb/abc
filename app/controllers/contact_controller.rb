class ContactController < ApplicationController
  ...
  def create
    @contact_form = ContactForm.new(contact_form_params.merge(referral: session[:utm]))
  ...
  def contact_form_params
    ...
    :country, :state, :curriculum, :grade, :school_district_size, :title_1, :returning_customer, :tax_exempt, :tax_exempt_id, :desired_dates, :desired_training_topic,
    :desired_training_city, :items_purchased, :format, :description, :school_district, :training_groups_size, :interested_in_hosting_events, :related_grade_module_unit_lession,
    grade_bands: [])
  end
end
