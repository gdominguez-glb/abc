class VideoGalleryController < ApplicationController
  ...
  def index
    ...
    @videos             = @videos.includes([video_group: [:products], taxons: [:taxonomy]]).order('custom_order asc, is_free desc, grade_order asc, module_order asc, lesson_order asc, title asc').page(params[:page]).per(24)
