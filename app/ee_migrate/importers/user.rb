  ...
  class User
    def self.import
      ...
      Migrate::Member.select([
        ...
        .joins("left join exp_channel_data ON exp_channel_titles.entry_id = exp_channel_data.entry_id group by exp_members.member_id")
        .group("exp_members.member_id").find_each do |member|
          user = Spree::User.find_or_initialize_by(email: member.email)
          user.update(
