class Spree::FavoriteProduct < ActiveRecord::Base
  ...
  scope :sort_by_taxons, ->(taxonomy_id) {
    ...
    joins("left JOIN spree_taxons ON spree_taxons.id = spree_products_taxons.taxon_id and spree_taxons.taxonomy_id = #{Spree::FavoriteProduct.sanitize(taxonomy_id)}").
